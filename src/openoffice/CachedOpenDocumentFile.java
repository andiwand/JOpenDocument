...
public class CachedOpenDocumentFile extends OpenDocumentFile {
	...
	private Cipher getFileCipher(Document manifestDocument, String path) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException {
		...
		for (Node fileEntry : manifestRoot.getChildNodes()) {
			if (fileEntry.findAttribute("full-path").getValue().equals(path)) {
				...
				if (encryptionData == null) {
					...
					String[] algorithmName = algorithmNode.findAttribute("algorithm-name").getValue().split(" ");
					String algorithm = algorithmName[0];
					String mode = algorithmName[1];
					String padding = "NoPadding";
					
					Cipher cipher = Cipher.getInstance(algorithm + "/" + mode + "/" + padding);
					cipher.init(Cipher.DECRYPT_MODE, new SimpleKey("asdf", algorithm));
					return cipher;
				}
			}
		}
